...
test('a note can be updated', async () => {
  ...
  const updatedNote = { ...noteToUpdate, content: 'Updated note' }

  await api.put(`/api/notes/${noteToUpdate.id}`).send(updatedNote).expect(200)

  const notesAtEnd = await helper.notesInDb()

  const contents = notesAtEnd.map(r => r.content)
  assert(contents.includes('Updated note'))
})
